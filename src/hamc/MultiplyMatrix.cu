...
__global__ void mult_kernel_compressed_data(HAMC_DATA_TYPE_t *A, HAMC_DATA_TYPE_t *B, HAMC_DATA_TYPE_t *C, int rowA, int rowB, int colA, int colB, int TILE_WIDTH)
{
    ...
    for(int i = 0; i < ((colA - 1)/(TILE_WIDTH * 4)) + 1; i++){
        ...
        for(int j = 0; j < 4; j++){
            //sharedB[] = B[];
            if(((threadIdx.y + (j + tilePos) * TILE_WIDTH) < rowB) && (Col < colB)){
                sharedB[threadIdx.x * 4 * TILE_WIDTH + j * TILE_WIDTH + threadIdx.y] = B[colB * (threadIdx.y + (j + tilePos) * TILE_WIDTH) + Col];
            }
            else{
        ...
        if(blockIdx.x == 0 && blockIdx.y == 0 && tid == 0){// && i == 0){
            ...
            for(int jk = 0; jk < 4; jk++){
                for(int q = 0; q < 32; q++){
                    for(int k = 0; k < 32; k++){
                        char bit = (sharedB[q * TILE_WIDTH + tid + k + jk * TILE_WIDTH]) & 1;
                        printf("%u,", bit);
                    }
                    printf("\n");
                }
